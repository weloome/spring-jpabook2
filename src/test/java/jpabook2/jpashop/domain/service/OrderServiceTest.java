...
import jpabook2.jpashop.repository.OrderRepository;
import jpabook2.jpashop.exception.NotEnoughStockException;
import jpabook2.jpashop.service.OrderService;
...
class OrderServiceTest {
    ...
    void enoughStock() throws Exception {
        //given
        Member member = createMember();
        ...
        assertThrows(
                ...
                "재고수량이 부족하면 NotEnoughStockException 예외가 발생해야 한다."
        );
    }
    ...
    private Member createMember() {
        ...
        member.setName("회원1");
        member.setAddress(new Address("김포", "장기", "123-123"));
        em.persist(member);
        return member;
    }
    ...
    private Book createBook(String name, int price, int sockQuantity) {
        ...
        book.setName(name);
        book.setPrice(price);
        book.setStockQuantity(sockQuantity);
        em.persist(book);
        return book;
    }
}